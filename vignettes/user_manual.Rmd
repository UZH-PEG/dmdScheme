...
* **columnName** Here enter the exact variable name used in the datasheet / database / tabular data. If the *columnData* *mappingColumn* combination refers to a condition for the whole data file, e.g. when only one species combination is used in the experiment fro this data set, than enter *NA*.
...
As the metadata as it is entered in the spreadsheet contains metadata for *many* different data files (as enterd in the column `DataFileMetaData / dataFileName`), the metadata needs to be split first into files containing only the metadata related to a specific data file. this is done by using the function:
