...
4. fixing any issues revealed by the validation step, and once satisfactorily validated 
5. deposit the metadata and data in a repository of your choice.
...
To get a copy of the spreadsheet to enter the data, either make a copy of [this googlesheets](https://docs.google.com/spreadsheets/d/1sCImO4VadNwNKuqgtC3CPksS95Tl6SJVtY78aFo9gLg) (File -- Make a copy) and delete the example data in the **green cells** wherer you can enter the metadata in, or you can run this R command to get an Excel version of the same but already empty sheet: 
...
You can always see a lidt of examples included by running the command
...
you will copy the example into your current working directory in a folder named `basic`.
...
* A spreadsheet file `expt1 emeScheme.gsheet` (identical to this [googlesheet](https://docs.google.com/spreadsheets/d/1sCImO4VadNwNKuqgtC3CPksS95Tl6SJVtY78aFo9gLg))
* A folder named `code` with two subfolders:
...
In the worked example (`basic`), open the `emeScheme.xlsx` file, or open the [googlesheet](https://docs.google.com/spreadsheets/d/1sCImO4VadNwNKuqgtC3CPksS95Tl6SJVtY78aFo9gLg).
...
After entering your data, you should do a validation of your data. The validation checks many different aspects of your data and returns a report (html, pdf, docx or all of trhe above) containing where your metadata is not consistent and wheer it dies not conform with the data. 

Before you can evaluate your data, you have to read it in R (I am using the xlsx supplied in the package as an example:

```{r eval = FALSE}
validate( x = system.file("googlesheet", "emeScheme.xlsx"), report = "html")
```
