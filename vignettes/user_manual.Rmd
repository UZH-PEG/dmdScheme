...
For some cells there are suggested entries available in a dropdown list. Please use these suggestions if you can. If the dropdown list contains an item that is three dots (...) you can enter an item not in the provided list: just type what you like in the cell. If you do this, you will get the message "Invalid:
Input must be an item on specified list". You can ignore this message. 
...
If you believe that other features of your experiment are important to include, please do so by adding the information in new rows and completing mandatory columns `valueProperty` (here put the name of the feature), `Description` (here describe what the feature is), and `DATA` (here put the value that the feature took in your experiment). 
...
You might think you should enter here the data file in which the measurements occur, but this information you will give in the *DataFileMetaData* worksheet.
