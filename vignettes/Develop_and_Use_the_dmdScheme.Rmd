...
```{r fig4, fig.show = "hold", out.width = "90%", fig.align = "center", fig.cap = "**Figure 4**: Example validation report, as cfrom the validation included in the `emeScheme` package.", echo = FALSE}
...
To discuss how a `dmdScheme` derived metadata scheme can be mapped (and exported) to another metadata scheme, we look at the example of the [EML scheme](https://eml.ecoinformatics.org) as it is widely used in ecology and the structural functionality is included in the `dmdScheme` package.

The actual mapping and development of the function to do the actual export is work in progress.

## General considerations

The `EML` scheme does provide sections to provide information about affectively all parts of the `dmdScheme`. Nevertheless, the `emeScheme` is a metadata scheme which is experiment based, i.e. can describe multiple datasets with multiple different methods, while the `EML` scheme is dataset based and can only contain one method. Terefore, to make this linking possible, we have to assume that the `dmdScheme` describes only one dataset. This can be either done by "physically" only describing one dataset per Spreadsheet, or by using a function which splits the `dmdScheme` which contains multiple datasets into per-dataset `dmdSchemes`. 

This second approach is implemented in the `emeScheme` in the function `emeSCheme_extract()` function, which extracts the data related to a single data file. This extraction can not be implemented in the `dmdScheme` package, as it is highly dependant on the metadata provided and how the different sheets relate to the data files.

In the same way, that the extract function can not be implemented in the general `dmdScheme`, the actual export to the `EML` scheme can also only be done in the specific scheme as it is also highly dependable on the metadata included. In addition, it can not be stated that **all** `dmdScheme` derived schemes can be exported to an `EML` scheme as all kind of data can be added to the scheme definition, and it is very likely that not all metadata in all `dmdScheme` derived schemes can be mapped to the `EML` scheme.

Finally, the aim of this mapping will not be a conversion to `EML` which can be re-imported into a `dmdScheme`, resulting in an identical object. This might be possible, but the aim would be simply to export the relevant metadata to `EML`, so that it can be indexed by repositories handling `EML` metadata schemes. The original `dmdScheme` metadata would be included in the data package.

## Approach and Mapping

To implement the export to `EML`, two functions need to be implemented:
1. splitting of the experiment based `dmdScheme` into individual data file based metadata sets
2. translation of the individual data file metadata sets to the  EML` format.

For the translation, a mapping needs to be done, which links metadata properties i the `dmdScheme` to metadata properties in the `EML` scheme. AS discussed above, this does not need to be a 1-1 linking, and the (theoretical?) re-import of the `EML` scheme does not necessarily need to re-create the original `dmdScheme` metadata set.

# Customize the index template

The aim of an index file is, to giva a short and concise overview over the data package deposited. It should contain certain metadata points, e.g. the DOI, authors, license, description, file name(s) of data file(s) and whatever else is considered necessary, to get a basic understanding of the dataset.
