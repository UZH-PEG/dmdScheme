...
test_that(
  ...
  {
    expect_error(
      object = emeSchemeToXml(x = addDataToEmeScheme(x = emeScheme_raw, s = emeScheme, dataCol = 1, verbose = FALSE), output = "nonsense"  ),
...
test_that(
  ...
  {
    expect_known_value(
      object = emeSchemeToXml(x = addDataToEmeScheme(x = emeScheme_raw, s = emeScheme, dataCol = 1, verbose = FALSE), output = "metadata"  ),
...
test_that(
  ...
  {
    expect_known_value(
      object = emeSchemeToXml(x = addDataToEmeScheme(x = emeScheme_raw, s = emeScheme, dataCol = 1, verbose = FALSE), output = "complete"  ),
  ...
  "emeSchemeToXml() raises warning with space in tag name",
  {
    expect_warning(
      object = emeSchemeToXml(
        x = addDataToEmeScheme(x = emeScheme_raw, s = emeScheme, dataCol = 1, verbose = FALSE),
        tag = "a test",
        output = "metadata"
      ),
