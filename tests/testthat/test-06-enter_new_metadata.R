...
test_that(
  "enter_new_metadata() raises error when schemeName is not specified",
  {
    expect_error(
...
test_that(
  ...
  {
    expect_equal(
      object = suppressWarnings(enter_new_metadata(schemeName = "dmdScheme", file = fn, .skipBrowseURL = TRUE)),
      expected = fn,
    )
  }
)
...
test_that(
  ...
  {
    expect_equal(
      object = suppressWarnings(enter_new_metadata(schemeName = "dmdScheme", file = fn, overwrite = TRUE, .skipBrowseURL = TRUE)),
      expected = fn,
    )
  }
)
...
test_that(
  ...
  {
    expect_true(
      object = file.exists( suppressWarnings(enter_new_metadata(schemeName = "dmdScheme", file = NULL, open = TRUE, overwrite = FALSE, .skipBrowseURL = TRUE) ) )
    )
  }
)
